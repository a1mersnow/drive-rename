...
(function (vue) {
  ...
  async function getFileListOfCurrentDir$2(parentId = getParentId$1()) {
    ...
    while (marker) {
      const { items, next_marker } = await post$1(listApi.toString().replace("%28", "(").replace("%29", ")"), {
        all: true,
        limit: PAGE_SIZE,
        drive_id: driveId,
        fields: "*",
