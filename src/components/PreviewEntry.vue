...
const { newName, oldName, done } = defineProps<{
  oldName: string
  newName: string
  done: boolean
}>()
...
<template>
  ...
  >
    ...
    <span v-if="!isSame" class="i-carbon:arrow-right justify-self-center" />
    <span v-else class="i-carbon:arrows-horizontal justify-self-center text-green-500" />
    <span :title="newName" class="truncate">{{ newName }} <span v-if="done">✅</span></span>
  </li>
</template>
